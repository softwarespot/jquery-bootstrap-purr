...
gulp.task('clean', function (cb) {
    del(['jquery-bootstrap-purr.min.js'], cb);
});

// Check the code meets the following standards outlined in .jshintrc
gulp.task('jshint', function () {
    return gulp.src('./jquery-bootstrap-purr.js')
        .pipe(jshint())
...
gulp.task('uglify', ['clean'], function () {
    return gulp.src('./jquery-bootstrap-purr.js')
        ...
        .pipe(rename('jquery-bootstrap-purr.min.js'))
        .pipe(gulp.dest('./'));
});

// Watch for changes to the main file to trigger hinting and minification
gulp.task('watch', function () {
    gulp.watch('./jquery-bootstrap-purr.js', ['jshint', 'uglify']);
});
